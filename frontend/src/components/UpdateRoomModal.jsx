...
const UpdateRoomModal = ({ isOpen, onClose, room, onRoomUpdated }) => {
  ...
  useEffect(() => {
    if (room && isOpen) {
      ...
      setImagePreview(room.imagePath ? getMediaUrl(room.imagePath) : null);
    }
  }, [room, isOpen]);
  ...
  const validateForm = () => {
    ...
    // Validación de imagen (opcional en actualización)
  ...
  const handleSubmit = async e => {
    ...
    const result = await Swal.fire({
      ...
      text: `¿Estás seguro de actualizar el espacio "${formData.name}"?`,
      icon: 'question',
    ...
    try {
      ...
      let errorMessage = 'Hubo un error al actualizar el espacio';
  ...
  const handleClose = () => {
    ...
    const hasData = Object.values(formData).some((value, key) => {
      if (key === 'imageFile') {
        return value !== null; // Solo verificar si hay una nueva imagen seleccionada
      }
      return value !== (room ? room[key] : '');
    });
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {formData.imageFile && (
                  <div className="mt-2 text-sm">
                    <p className="text-green-600">
                      ✓ Nueva imagen seleccionada: {formData.imageFile.name}
                    </p>
                ...
                <p className="mt-1 text-xs text-gray-500">
                  ...
                  (Opcional - deja vacío para mantener la imagen actual)
                </p>
              </div>
